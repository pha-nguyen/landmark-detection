...
import numbers, numpy as np, pickle
...
param = snapshot['args']
eval_transform = transforms.Compose([transforms.PreCrop(param.pre_crop_expand), transforms.TrainScale2WH(
    (param.crop_width, param.crop_height)), transforms.ToTensor(), normalize])
...
def evaluate(args):
    ...
    [image, _, _, _, _, _, cropped_size], meta = dataset.prepare_input(
        args.image, args.face)
    # network forward
    with torch.no_grad():
        ...
        print(
            'IN-shape : {:}, FLOPs : {:} MB, Params : {:} MB'.format(list(inputs.shape), flops, params))
    ...
    locations, scores = np_batch_locs[0, :-1,
    ...
    for i in range(param.num_pts):
        point = prediction[:, i]
        print('the {:02d}/{:02d}-th point : ({:.1f}, {:.1f}), score = {:.2f}'.format(
            i, param.num_pts, float(point[0]), float(point[1]), float(point[2])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image',            type=str,
                        help='The evaluation image path.')
    parser.add_argument('--model',            type=str,
                        help='The snapshot to the saved detector.')
    parser.add_argument('--face',  nargs='+', type=float,
                        help='The coordinate [x1,y1,x2,y2] of a face')
    parser.add_argument('--save',             type=str,
                        help='The path to save the visualized results.')
